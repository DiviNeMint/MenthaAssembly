...
{
    public sealed unsafe class AdjustGammaPixelAdapter<T> : PixelAdapter<T>
...
#pragma warning restore IDE0044 // 新增唯讀修飾元
        ...
        private AdjustGammaPixelAdapter(AdjustGammaPixelAdapter<T> Adapter)
        {
            X = Adapter.X;
            Y = Adapter.Y;
        ...
        public AdjustGammaPixelAdapter(IPixelAdapter Adapter, double Gamma)
        {
            X = Adapter.X;
            Y = Adapter.Y;
            XLength = Adapter.XLength;
        ...
        private byte Adjust(byte Value)
        {
            ...
            Result = (byte)(Math.Pow(Value / 255.0, Gamma) * 255.0);
            Cache[Value] = Result;
            return Result;
        }
        ...
        public override PixelAdapter<T> Clone()
            => new AdjustGammaPixelAdapter<T>(this);
